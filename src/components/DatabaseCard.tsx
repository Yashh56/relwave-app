    ...
    <>
          ...
          <div className="flex items-start justify-between">
            <Link to={`/${id}`} className="flex items-center gap-3 min-w-0 flex-1 pr-1">
              ...
              <div className="truncate min-w-0 flex-1">
                <CardTitle className="text-xl mb-1 text-foreground group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors truncate">
                  {name}
                </CardTitle>
            ...
            <div className="flex items-center gap-1 shrink-0 ml-1">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-popover border-border text-foreground shadow-lg">
                  <DropdownMenuItem onClick={handleTest} className="cursor-pointer hover:bg-accent focus:bg-accent">
                    <TestTube className="h-4 w-4 mr-2 text-cyan-600 dark:text-cyan-400" />
                    Test Connection
                  </DropdownMenuItem>
                  <DropdownMenuSeparator className="bg-border" />
                  <DropdownMenuItem onClick={handleDelete} className="cursor-pointer text-destructive hover:bg-destructive/10 focus:bg-destructive/10">
          ...
          <CardContent className="p-4 sm:p-6 pt-0">
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between text-sm border-b border-border pb-2">
                ...
                <span className="font-mono font-medium text-foreground px-2 py-0.5 rounded-md bg-accent truncate max-w-[50%]">
                  {type}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-mono font-medium text-lg text-foreground">
                  {result ? parseInt(result.stats.total_tables) : 0}
                </span>
              </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="bg-background border-border text-foreground">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-accent border-border text-foreground hover:bg-muted">
              Cancel
            </AlertDialogCancel>
