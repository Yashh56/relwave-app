...
export const useIndexPage = (bridgeReady: boolean) => {
  ...
  const { status, totalSize, totalTables, connectedCount, showStatsLoading, refetchStatus } =
  ...
  const [prefilledConnectionData, setPrefilledConnectionData] = useState<
  ...
  const selectedDatabase = useMemo(
    () =>
      selectedDb
        ? (databases.find((db: DatabaseConnection) => db.id === selectedDb) ?? null)
        : null,
  ...
  const filteredDatabases = useMemo(
    () =>
      databases.filter((db: DatabaseConnection) => {
        const matchesSearch =
  ...
  const handleAddDatabase = async (formData: ConnectionFormData) => {
    ...
    if (formData.useSsh) {
      ...
      if (formData.sshAuthMethod === "password" && !formData.sshPassword)
        sshMissing.push("SSH Password");
      if (formData.sshAuthMethod === "privateKey" && !formData.sshPrivateKeyPath)
        sshMissing.push("SSH Private Key Path");

      if (sshMissing.length) {
        toast.error("Missing SSH fields", {
          description: `Please fill in: ${sshMissing.join(", ")}`,
        });
        return;
      }
    }
