  ...
  if (isLoading)
    return (
      <div className="p-3 text-sm text-muted-foreground flex items-center gap-2">
        <Spinner className="w-4 h-4" /> Loading tables for {db.name}...
      </div>
    );
        ...
        {tables.map((t) => (
          ...
          >
            ...
            <span className="truncate">
              {t.schema !== "public" && t.schema !== db.database ? `${t.schema}.` : ""}
              {t.name}
            </span>
          </div>
        ))}
                ...
                {itemsToShow.map((db, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <Database className={cn("h-5 w-5", getDbColors(db.type).text)} />
                      </div>
                ...
                {Array.from({ length: emptySlots }).map((_, i) => (
                  <div
                    ...
                    className={cn(
                      ...
                      (i !== emptySlots - 1 || recentDatabases.length > 5) &&
                        "border-b border-border/30",
                    )}
                  />
                ))}
