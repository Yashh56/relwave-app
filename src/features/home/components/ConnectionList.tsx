        ...
        <ContextMenuTrigger asChild>
          ...
          >
            <div
              {...attributes}
              {...listeners}
              className="p-1 hover:bg-muted rounded opacity-0 group-hover/item:opacity-100 transition-opacity cursor-grab active:cursor-grabbing"
            >
            ...
            <div
              className={cn(
                ...
                isConnected
                  ? "bg-emerald-500 ring-emerald-500/10 motion-safe:animate-pulse"
  ...
  return (
    ...
    >
      <div className="flex items-center group/header px-2 py-1.5">
        <button onClick={() => onToggle(group.id)} className="p-1 hover:bg-accent rounded mr-1">
          {group.isCollapsed ? (
...
export function ConnectionList({
  ...
  const ungroupedConnections = useMemo(() => {
    return ungroupedIds
      .map((id) => filteredDatabasesById.get(id))
      .filter(Boolean) as DatabaseConnection[];
              ...
              >
                {groups.length > 0 && (
                  <div className="px-2 mb-2">
                    <span className="text-xs text-muted-foreground/50 lowercase">ungrouped</span>
                  </div>
                )}
                <div className="space-y-0.5">
                  <SortableContext
                    items={ungroupedConnections.map((c) => c.id)}
                    strategy={verticalListSortingStrategy}
                  >
                ...
                <div className="mt-4 pt-2 border-t border-border/20 rounded-lg">
                  ...
                  <div className="space-y-0.5">
                    {unlinkedProjects.map((project) => (
                      <UnlinkedProjectItem key={project.id} project={project} />
                    ))}
                  </div>
                </div>
              )}
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 bg-sidebar border border-primary/40 shadow-2xl rounded-md min-w-56 backdrop-blur-xl ring-1 ring-white/5">
                      ...
                      <div
                        className={cn(
                          ...
                          status.get(activeId) === "connected"
                            ? "bg-emerald-500 ring-emerald-500/20 motion-safe:animate-pulse"
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <Card className="text-center p-2 rounded-md premium-card shadow-sm">
            <CardContent className="px-2 pb-0">
              ...
              <CardDescription className="text-[10px] text-muted-foreground uppercase font-bold tracking-tight text-center">
                Online
              </CardDescription>
            </CardContent>
          </Card>
