          ...
          {step === 1 ? (
            <div className="space-y-4">
              ...
              >
                ...
                <div
                  className="flex items-start space-x-3 rounded-md border border-destructive/20 bg-destructive/5 p-3 cursor-pointer"
                  onClick={() => setChoice("delete_project")}
                >
                  <RadioGroupItem value="delete_project" id="delete_project" className="mt-1" />
                  <Label
                    htmlFor="delete_project"
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm font-medium text-destructive mt-4">This cannot be undone.</p>

              {hasGitRemote && gitRemoteUrl && (
                <div className="mt-4 p-3 rounded-md bg-muted border text-sm">
                  <span className="font-semibold flex items-center gap-1 mb-1">⚠ Note</span>
                  <span className="text-muted-foreground">
                    The remote repository at{" "}
                    <code className="bg-background px-1 py-0.5 rounded">{gitRemoteUrl}</code> will
                    not be deleted. Only the local project folder will be removed.
                  </span>
                </div>
              )}
            </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0 mt-4">
          {step === 1 ? (
            <>
              ...
              <Button
                variant={choice === "delete_project" ? "destructive" : "default"}
                onClick={handleNext}
                disabled={isDeleting}
              >
