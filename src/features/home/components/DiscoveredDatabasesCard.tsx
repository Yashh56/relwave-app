    ...
    <div className="mb-6">
      ...
      {isScanning && databases.length === 0 ? (
        <div className="rounded-lg border border-border/50 bg-card/80 p-8 flex flex-col items-center justify-center shadow-sm">
          ...
          <p className="text-sm text-muted-foreground">Scanning local ports...</p>
        </div>
      ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <CardContent className="flex-1 min-w-0 pb-0">
                    <div className="flex items-center gap-2 mb-1">
                      <CardTitle
                        className={cn("text-xs font-medium uppercase tracking-wide", colors.text)}
                      >
                        {db.type}
                      </CardTitle>
                      <CardDescription
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-border/40">
                      <CardDescription className="text-xs text-muted-foreground truncate pr-2">
                        Suggested:{" "}
                        <span className="font-medium text-foreground">{db.suggestedName}</span>
                      </CardDescription>

                      {existingConnections.some(
                        (c) => c.host === db.host && String(c.port) === String(db.port),
                      ) ? (
