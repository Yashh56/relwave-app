    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create indexes for table <span className="font-mono font-medium">{tableName}</span> in
            schema <span className="font-mono font-medium">{schemaName}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {indexes.length === 0 ? (
              <div className="border border-dashed border-border/50 rounded-lg p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-2">No indexes defined yet</p>
          ...
          <div className="border border-blue-500/20 bg-blue-500/5 rounded-lg p-4">
            <p className="text-sm text-muted-foreground">
              <strong className="text-foreground">💡 Tip:</strong> You can add indexes now or skip
              and add them later. Indexes improve query performance but may slow down write
              operations.
            </p>
          </div>
        </div>
