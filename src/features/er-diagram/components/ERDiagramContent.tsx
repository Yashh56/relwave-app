...
const ERDiagramContent: React.FC<ERDiagramContentProps> = ({ nodeTypes, projectId }) => {
  ...
  const { schemaData, savedLayout, isLoading, dataSource, hasLiveSchema, syncFromDatabase } =
  ...
  const availableSchemas = useMemo(() => {
    ...
    return schemaData.schemas.filter((s) => s.tables?.length > 0).map((s) => s.name);
  }, [schemaData]);
  ...
  const handleExport = useCallback(
    async (format: ExportFormat) => {
      ...
      const flowContainer =
        annotationMode && diagramContainerRef.current
              ...
              <DropdownMenuContent align="start">
                <DropdownMenuItem onClick={() => setSelectedSchema("__all__")}>
                  ...
                  <span className="ml-auto text-xs text-muted-foreground">
                    {schemaData?.schemas.reduce((acc, s) => acc + (s.tables?.length || 0), 0)}{" "}
                    tables
                  </span>
                </DropdownMenuItem>
                {availableSchemas.map((schema) => (
                  <DropdownMenuItem key={schema} onClick={() => setSelectedSchema(schema)}>
                    {schema}
                    <span className="ml-auto text-xs text-muted-foreground">
                      {schemaData?.schemas.find((s) => s.name === schema)?.tables?.length || 0}{" "}
                      tables
                    </span>
                  </DropdownMenuItem>
                ))}
            ...
            {searchQuery && (
              <div className="absolute mt-1 text-xs text-muted-foreground">
                ...
                {filteredNodes.length > 0 && (
                  <button onClick={fitToFiltered} className="ml-2 text-primary hover:underline">
                    Focus
                  </button>
                )}
              </div>
        ...
        {hoveredEdge && hoveredEdge.data && (
          <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-popover border border-border rounded-lg shadow-lg p-3 text-xs z-50">
            ...
            <div className="text-muted-foreground">
              {hoveredEdge.data.sourceTable}.{hoveredEdge.data.sourceColumn} →{" "}
              {hoveredEdge.data.targetTable}.{hoveredEdge.data.targetColumn}
            </div>
            <div className="flex gap-3 mt-1 text-[10px]">
              <span>
                ON DELETE: <span className="text-red-500">{hoveredEdge.data.deleteRule}</span>
              </span>
              <span>
                ON UPDATE: <span className="text-blue-500">{hoveredEdge.data.updateRule}</span>
              </span>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-border bg-card px-4 py-2">
        <div className="container mx-auto flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-3">
            {annotationMode && <span className="text-primary font-medium">Annotation Mode</span>}
            {savedLayout && <span className="text-muted-foreground/60">Layout saved</span>}
            <span>
              {annotationMode
