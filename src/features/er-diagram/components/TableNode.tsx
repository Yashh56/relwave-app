...
const TableNode: React.FC<{ data: TableNodeData }> = ({ data }) => {
  ...
  const uniqueColumns = new Set(data.uniqueConstraints?.map((uc) => uc.column_name) || []);
  ...
  const fkSourceColumns = new Set(data.foreignKeys?.map((fk) => fk.source_column) || []);

  // Map FK source to target info for tooltips
  const fkTargetMap = new Map(data.foreignKeys?.map((fk) => [fk.source_column, fk]) || []);

  // Columns that are referenced by other tables (targets of FKs)
  const pkColumns = new Set(data.columns.filter((col) => col.isPrimaryKey).map((col) => col.name));
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div
        className={`min-w-56 shadow-md border border-border rounded-lg bg-card relative ${highlightClass} transition-all duration-200 overflow-hidden`}
      >
        {/* Header */}
          ...
          {visibleColumns.map((col) => {
            ...
            const tooltipContent = (
              <div className="text-xs space-y-1 max-w-62.5">
                <div className="font-semibold border-b pb-1 mb-1 dark:text-white text-black">
                  {col.name}
                </div>
                <div className="text-muted-foreground">
                  <span>Type:</span> {col.type}
                </div>
                <div className="text-muted-foreground">
                  <span>Nullable:</span> {col.nullable ? "Yes" : "No"}
                </div>
                {col.defaultValue && (
                  <div>
                    <span className="text- dark:text-muted-foreground">Default:</span>{" "}
                    {col.defaultValue}
                  </div>
                )}
                {col.isPrimaryKey && <div className="text-amber-500">🔑 Primary Key</div>}
                {fkInfo && (
                  <div className="text-cyan-500 border-t pt-1 mt-1">
                    <div>
                      → References: {fkInfo.target_table}.{fkInfo.target_column}
                    </div>
                ...
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      ...
                      <span
                        className={`truncate ${col.isPrimaryKey ? "font-medium" : col.isForeignKey ? "text-cyan-600 dark:text-cyan-400 font-medium" : "text-foreground"}`}
                      >
                        {col.name}
                      </span>
