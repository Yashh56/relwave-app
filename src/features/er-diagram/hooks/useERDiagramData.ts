...
export function useERDiagramData(
  ...
  // ---- Live DB schema ----
  const { data: liveSchema, isLoading: liveLoading, error: liveError } = useFullSchema(dbId);

  // ---- Project files ----
  const { data: projectSchemaFile, isLoading: projectSchemaLoading } = useProjectSchema(
    projectId ?? undefined,
  );

  const { data: savedLayout, isLoading: erLayoutLoading } = useProjectERDiagram(
    projectId ?? undefined,
  );
