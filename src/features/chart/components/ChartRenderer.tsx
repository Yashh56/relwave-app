...
const ChartRendererComponent = ({ chartType, xAxis, yAxis: _yAxis, data }: ChartRendererProps) => {
      ...
      <ChartContainer config={chartConfig} className="h-80 w-full">
        <BarChart {...commonProps} barCategoryGap="28%">
          ...
          <Bar dataKey="value" radius={[6, 6, 0, 0]} maxBarSize={56}>
            {chartData.map((_, i) => (
              <Cell key={i} fill={PALETTE_VARS[i % PALETTE_VARS.length]} fillOpacity={0.88} />
            ))}
          </Bar>
        </BarChart>
      </ChartContainer>
    ...
    return (
      <ChartContainer config={chartConfig} className="h-80 w-full">
        <PieChart>
          ...
          >
            {top.map((_, i) => (
              <Cell key={i} fill={PALETTE_VARS[i % PALETTE_VARS.length]} fillOpacity={0.9} />
            ))}
            <LabelList
              dataKey="name"
              position="outside"
          ...
          <ChartLegend
            content={
              <ChartLegendContent
                nameKey="name"
                className="text-[10px] flex-wrap gap-x-4 gap-y-1.5 mt-2"
              />
            }
          />
        </PieChart>
