...
export const ColumnTreeItem = ({
  ...
  <ContextMenu>
          ...
          <div className="mb-0.5">
            ...
            >
              <div className="flex items-center gap-1 flex-1 min-w-0">
                {column.isPrimaryKey && (
                  <Key className="h-3 w-3 text-amber-600 dark:text-amber-400 shrink-0" />
                )}
                {column.isForeignKey && (
                  <Link2 className="h-3 w-3 text-cyan-600 dark:text-cyan-400 shrink-0" />
                )}
                <span
    ...
    <ContextMenuContent className="bg-popover border-border">
      <ContextMenuItem
        onClick={() => onCopy(column.name, "Column name")}
      ...
      <ContextMenuItem
        onClick={() => onCopy(column.type, "Column type")}
        className="hover:bg-accent"
      >
        <Copy className="h-4 w-4 mr-2" />
        Copy Type
