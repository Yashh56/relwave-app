  ...
  return (
    <ContextMenu>
            ...
            <div className="ml-6 mt-1">
              {table.columns.map((column: Column) => (
                <ColumnTreeItem
                  ...
                  column={column}
                  isSelected={
                    selectedItem === `${dbName}:::${schemaName}:::${table.name}:::${column.name}`
                  }
                  fkInfo={getFkInfo(column, table.foreignKeys)}
                  isIndexed={
                    !!table.indexes?.some(
                      (idx: any) => idx.column_name === column.name && !idx.is_primary,
                    )
                  }
                  isUnique={
                    !!table.uniqueConstraints?.some((uc: any) => uc.column_name === column.name)
                  }
                  onSelect={() =>
                    onSelectItem(`${dbName}:::${schemaName}:::${table.name}:::${column.name}`)
                  }
                  onCopy={onCopy}
                />
              ))}
      ...
      <ContextMenuContent className="bg-popover border-border">
        ...
        <ContextMenuItem
          onClick={() => onCopy(table.name, "Table name")}
