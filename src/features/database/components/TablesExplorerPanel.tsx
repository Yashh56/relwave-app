    ...
    <div className="h-full min-h-0 w-full flex flex-col bg-transparent overflow-hidden">
      ...
      <ScrollArea className="flex-1 min-h-0 overflow-hidden">
        <div className="p-2 space-y-1">
          {loading ? (
            <div className="text-center py-8 text-sm text-muted-foreground">Loading tables...</div>
          ) : filteredTables.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No tables found</div>
          ) : (
            filteredTables.map((table) => (
              <ContextMenu key={`${table.schema}.${table.name}`}>
                ...
                <ContextMenuContent>
                  ...
                  <ContextMenuItem
                    onClick={() => setDropTableData(table)}
