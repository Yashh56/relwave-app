...
describe("MonitoringWebSocketServer", () => {
  ...
  test("closes unsupported websocket connections immediately", async () => {
    ...
    (dbService.getDatabaseConnection as any).mockResolvedValue({
      conn: { id: "conn" },
  ...
  test("sends snapshots over websocket and enforces minimum interval", async () => {
    ...
    await wsServer.emit("connection", socket, {
      url: "/monitoring?dbId=db-1&intervalMs=1000",
    } as any);

    expect(dbService.getDatabaseConnection).toHaveBeenCalledWith("db-1");
    expect(monitoringService.getSnapshot).toHaveBeenCalledWith(
      "db-1",
      { id: "conn" },
      DBType.POSTGRES,
    );
