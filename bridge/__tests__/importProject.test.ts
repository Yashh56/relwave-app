...
async function createSourceProject(opts: {
  ...
  if (!opts.skipRelwaveJson) {
    ...
    await fs.writeFile(path.join(SOURCE_DIR, "relwave.json"), JSON.stringify(meta, null, 2));
  }
...
describe("ProjectStore — Import Project", () => {
  ...
  describe("parseEnvFile", () => {
    test("should parse simple KEY=VALUE pairs", () => {
      const result = ProjectStore.parseEnvFile("DB_HOST=localhost\nDB_PORT=5432\nDB_USER=admin");
  ...
  describe("scanImportSource", () => {
    ...
    test("should throw when relwave.json has no name", async () => {
      ...
      await expect(store.scanImportSource(SOURCE_DIR)).rejects.toThrow("Invalid relwave.json");
    });
  ...
  describe("importProject", () => {
    ...
    test("should throw when databaseId is missing", async () => {
      ...
      await expect(store.importProject({ sourcePath: SOURCE_DIR, databaseId: "" })).rejects.toThrow(
    ...
    test("should NOT modify tracked files (read-only import)", async () => {
      ...
      const origQueries = await fs.readFile(
        path.join(SOURCE_DIR, "queries", "queries.json"),
        "utf-8",
      );
    ...
    test("should store databaseId in local config (git-ignored)", async () => {
      ...
      const meta = JSON.parse(await fs.readFile(path.join(SOURCE_DIR, "relwave.json"), "utf-8"));
  ...
  describe("linkDatabase", () => {
    ...
    test("should update schema file's databaseId", async () => {
      ...
      // Save a schema first
      await store.saveSchema(project.id, [{ name: "public", tables: [] }]);
