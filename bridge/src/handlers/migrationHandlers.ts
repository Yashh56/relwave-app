...
export class MigrationHandlers {
  ...
  async handleGenerateCreateMigration(params: any, id: number | string) {
    try {
      ...
      const { generateCreateTableMigration, writeMigrationFile } =
        await import("../utils/migrationGenerator");
  ...
  async handleGenerateAlterMigration(params: any, id: number | string) {
    try {
      ...
      // Generate migration file
      const { generateAlterTableMigration, writeMigrationFile } =
        await import("../utils/migrationGenerator");
  ...
  async handleGenerateDropMigration(params: any, id: number | string) {
    try {
      ...
      // Generate migration file
      const { generateDropTableMigration, writeMigrationFile } =
        await import("../utils/migrationGenerator");
          ...
          if (projectDir) {
            ...
            if (syncResult.error) {
              this.logger?.warn(
                ...
                "Git sync failed after migration apply",
              );
            }
          }
        }
    ...
    try {
      ...
      if (!dbId && projectId) {
        ...
        if (!project?.databaseId) {
          return this.rpc.sendError(id, {
            code: "BAD_REQUEST",
            message: "Project has no linked database",
          });
        }
      ...
      if (!dbId)
        return this.rpc.sendError(id, {
          code: "BAD_REQUEST",
          message: "Missing dbId or projectId",
        });
      ...
      for (const migration of pending) {
        const migrationFile = (await fs.promises.readdir(migrationsDir)).find((f) =>
          f.startsWith(migration.version),
        );
        if (!migrationFile)
      ...
      try {
        ...
        if (project) {
          ...
          await writeMigrationLock(
            dbId,
            schemaHash,
            updatedApplied.map((m: any) => m.version),
          );
        }
      } catch (syncErr) {
  ...
  async handleApplySnapshot(params: any, id: number | string) {
    try {
      ...
      if (dbType === "mysql") await this.queryExecutor.mysql.applyMigration(conn, baselinePath);
        ...
        await this.queryExecutor.postgres.applyMigration(conn, baselinePath);
      else if (dbType === "mariadb")
        await this.queryExecutor.mariadb.applyMigration(conn, baselinePath);
      else if (dbType === "sqlite")
        await this.queryExecutor.sqlite.applyMigration(conn, baselinePath);
        ...
        if (project) {
          ...
          if (projectDir) {
            ...
            if (syncResult.error) {
              this.logger?.warn(
                { error: syncResult.error },
                "Git sync failed after migration rollback",
              );
            }
          }
        }
  ...
  async handleGetMigrationSQL(params: any, id: number | string) {
    try {
      ...
      const { listMigrationFiles, readMigrationFile } =
        await import("../utils/migrationFileReader");
      const files = listMigrationFiles(migrationsDir);
      const migrationFile = files.find((f) => f.startsWith(version));
