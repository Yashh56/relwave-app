...
`;
  ...
  for (const schema of snapshot.schemas) {
    ...
    for (const table of schema.tables) {
      ...
      if (
        ["schema_migrations", "__relwave_migrations", "relwave_migrations"].includes(table.name)
      ) {
        continue;
      }
      ...
      const tableRef =
        dbType === "sqlite"
          ...
          : `${quoteIdent(schema.name, dbType)}.${quoteIdent(table.name, dbType)}`;
      ...
      if (dbType === "postgresql") {
        for (const col of table.columns) {
          if (col.defaultValue && col.defaultValue.includes("nextval(")) {
            ...
            const seqMatch =
              col.defaultValue.match(/nextval\('([^']+)'/i) ||
              col.defaultValue.match(/nextval\("([^"]+)"/i);
            if (seqMatch && seqMatch[1]) {
              const seqName = seqMatch[1];
              const isSchemaQualified = seqName.includes(".");
              const seqRef = isSchemaQualified
                ? seqName
                : `${quoteIdent(schema.name, dbType)}.${quoteIdent(seqName, dbType)}`;
      ...
      const fkDefs = (table.foreignKeys || []).map((fk) => {
        ...
        const targetRef =
          dbType === "sqlite"
            ? quoteIdent(fk.referencedTable, dbType)
            : `${quoteIdent(fk.referencedSchema || schema.name, dbType)}.${quoteIdent(fk.referencedTable, dbType)}`;
    ...
    if (schema.views) {
      for (const v of schema.views) {
        const viewRef =
          dbType === "sqlite"
            ? quoteIdent(v.name, dbType)
            : `${quoteIdent(schema.name, dbType)}.${quoteIdent(v.name, dbType)}`;
