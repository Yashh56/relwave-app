...
export class GitService {
  ...
  async getStatus(dir: string): Promise<GitStatus> {
    ...
    try {
      ...
      const branchOutput = await this.git(dir, "status", "--porcelain=v2", "--branch");
    ...
    try {
      const statusOutput = await this.git(dir, "status", "--porcelain=v1", "-uall");
  ...
  async log(dir: string, count = 20): Promise<GitLogEntry[]> {
    try {
      ...
      const output = await this.git(dir, "log", `--max-count=${count}`, `--format=${format}`);
  ...
  async ensureGitignore(dir: string): Promise<boolean> {
    ...
    if (fsSync.existsSync(gi)) {
      ...
      if (!existing.includes("relwave.local.json")) {
        fsSync.appendFileSync(gi, "\n\n" + this.generateGitignore(), "utf-8");
  ...
  > {
    try {
      const output = await this.git(dir, "blame", "--porcelain", "--", filePath);
      if (!output) return [];
  ...
  async stashList(dir: string): Promise<{ index: number; message: string; date: string }[]> {
    try {
      const SEP = "<<SEP>>";
      const output = await this.git(dir, "stash", "list", `--format=%gd${SEP}%s${SEP}%aI`);
      if (!output) return [];
